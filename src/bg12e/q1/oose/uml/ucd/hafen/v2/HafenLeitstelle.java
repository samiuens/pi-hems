...
public class HafenLeitstelle {
    ...
    public List<Schiff> sucheLadungSchiffe(int code) {
        ...
        for(Schiff s : this.schiffe) {
            if(s instanceof Fschiff && ((Fschiff) s).getLadung() == code) {
                schiffe.add(s);
            }
        }
        return schiffe;
    ...
    public void aktualisiereLogeintrag(String schiffname) {
        Schiff s = this.sucheSchiff(schiffname);
        int logIndex = -1;
        Logeintrag log = null;

        for(int i = 0; i < this.logs.size(); i++) {
            Logeintrag logEintrag = this.logs.get(i);
            if(logEintrag.getSchiffname().equals(s.getName())) {
                logIndex = i;
                break;
            }
        }
        if(logIndex == -1) {
            log = new Logeintrag(schiffname, "", -1);
        } else {
            log = this.findeLogeintrag(logIndex);
        }

        if(s.getSeestrasse() != null) log.setBezeichnungSeestrasse(s.getSeestrasse().getBezeichnung());
        if(s.getLiegeplatz() != null) log.setNrLiegeplatz(s.getLiegeplatz().getNummer());
    }
    ...
    public boolean zuweisenLiegeplatz(Schiff s) {
        for(Liegeplatz lp : this.liegeplaetze) {
            if(lp.istFrei()) {
                if(s instanceof Fschiff && !lp.getLadungsarten().contains(((Fschiff) s).getLadung())) {
